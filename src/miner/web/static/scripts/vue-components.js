    ...
    methods: {
        download: function (name) {
            ...
            this.$fetch(
                ...
                .then(json => {
                    Object.assign(this.models.find(x => x.name == name), json.find(x => x.name == name));
        ...
        update_models_status: function () {
            this.$fetch(`/api/${this.component}/models/status`, { method: "GET" })
                .then(json => {
                    if (this.model)
                        if (this.models)
                            Object.assign(this.models.find(x => x.name == this.model), json.find(x => x.name == this.model));
                        else
                            this.models = [json.find(x => x.name == this.model)]
                    else
                        this.models = json;
                })
                .catch(error => {
        ...
        <div v-if="model.is_loaded" class="mr-5">
            ...
            <div class="d-inline-block align-top">
                <button v-on:click.stop.prevent="download(model.name)" type="button" class="btn btn-sm btn-outline-secondary">
                    ...
                    <span v-if="download_clicked[model.name] || model.is_downloading"><span v-if="model.download_percentage"> {{model.download_percentage}}%</span></span>
                </button>
